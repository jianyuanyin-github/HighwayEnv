...
class plotter:
    ...
    def __init__(self, table, smax, r, lencar, ppm):

        self.r = r
        self.lencar = lencar
        self.smax = smax
        self.ppm = ppm
        maxidx = np.floor(smax * self.ppm).astype(np.int32)
        # downsample
        self.downsampling = 2
        ...
        self.fig2, (self.ax1, self.ax2) = plt.subplots(
        ...
        self.track_xbounds = [
            -0.1 - np.max(r) + np.min(self.coords_full[:, 0]),
            np.max(self.coords_full[:, 0]) + np.max(r) + 0.1,
        ]
        self.track_ybounds = [
            -0.1 - np.max(r) + np.min(self.coords_full[:, 1]),
            np.max(self.coords_full[:, 1]) + np.max(r) + 0.1,
        ]
    ...
    def plot_track(self):
        ...
        for idx_center in range(len(self.coords)):
            for idx_bound_unsigned in range(100):
                idx_bound = np.mod(
                    idx_center + idx_bound_unsigned - 50, len(self.coords)
                )
                ...
                if dx1**2 + dy1**2 < self.r**2 - 0.1:  # 从0.003改为0.1
                    bounds_to_del_1.append(idx_bound)
                if dx2**2 + dy2**2 < self.r**2 - 0.1:  # 从0.003改为0.1
    ...
    def plot_horizon(self, thetavals, xval):
        ...
        tr = matplotlib.transforms.Affine2D().rotate_deg_around(
            xval[0, 0], xval[0, 1], phi0 * 180 / 3.14159
        )
        ts = self.ax.transData
        t = tr + ts

        carrect = patches.Rectangle(
            (xval[0, 0] - width / 2, xval[0, 1] - height / 2),
            width,
            height,
            linewidth=1,
            edgecolor="k",
        ...
        self.cardir = self.ax.plot(
            [center[0], front[0]], [center[1], front[1]], linewidth=1, color="r"
        )

        self.theta_horz = self.ax.scatter(
            x_theta_vals, y_theta_vals, marker="x", s=4, color="g"
        )
    ...
    def plot_agents(self, agent_1, agent_2):
        ...
        self.cardir1 = self.ax.plot(
            [center[0], front[0]], [center[1], front[1]], linewidth=1, color="k"
        )
        self.pos_horz1 = self.ax.plot(
            agent_1[:, 3], agent_1[:, 4], linewidth=1, color="r"
        )
        ...
        self.cardir2 = self.ax.plot(
            [center[0], front[0]], [center[1], front[1]], linewidth=1, color="k"
        )
        self.pos_horz2 = self.ax.plot(
            agent_2[:, 3], agent_2[:, 4], linewidth=1, color="b"
        )
    ...
    def plot_input_state_traj(self, zval, varnames):
        ...
        for idx in range(zval.shape[1] - 3):
            temp = self.ax1.step(
                time, zval[:, idx + 3], label=varnames[idx + 3], where="post"
            )
    ...
    def plot_static_obstacle(self, x, y, phi, l, w, color):
        obstacle = patches.Ellipse(
            (x, y),
            l,
            w,
            angle=phi * 180 / 3.14159,
            linewidth=1,
            edgecolor=color,
            facecolor="None",
        )
...
def compute_objective(
    Ts, Qc, Ql, Q_theta, R_a, R_delta, theta, theta_hat, x, u, xt, phit
):
    xt_hat = xt[:, 0] + np.multiply(np.cos(phit), (theta - theta_hat))
    yt_hat = xt[:, 1] + np.multiply(np.sin(phit), (theta - theta_hat))
    e_cont = np.multiply(np.sin(phit), (xt_hat - x[:, 0])) - np.multiply(
        np.cos(phit), (yt_hat - x[:, 1])
    )
    e_lag = np.multiply(np.cos(phit), (xt_hat - x[:, 0])) + np.multiply(
        np.sin(phit), (yt_hat - x[:, 1])
    )
    thetadot = u[:, 2]
    deltadot = u[:, 1]
    jerk = u[:, 0]
    ...
    for stageidx in range(stages):
        objective += (
            (e_cont[stageidx] ** 2) * Qc
            + (e_lag[stageidx] ** 2) * Ql
            - Q_theta * thetadot[stageidx]
            + (jerk[stageidx] ** 2) * R_a
            + (deltadot[stageidx] ** 2) * R_delta
        )
...
class replay_plotter:
    ...
    def __init__(self, table, smax, r, lencar, ppm=100):
