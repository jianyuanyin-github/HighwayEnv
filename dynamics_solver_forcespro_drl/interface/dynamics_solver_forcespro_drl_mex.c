...
#include "../include/dynamics_solver_forcespro_drl_memory.h"
...
void copyCArrayToM_dynamics_solver_forcespro_drl_float(dynamics_solver_forcespro_drl_float *src, double *dest, solver_int32_default dim) 
{
    solver_int32_default i;
    for( i = 0; i < dim; i++ ) 
    {
...
void copyMArrayToC_dynamics_solver_forcespro_drl_float(double *src, dynamics_solver_forcespro_drl_float *dest, solver_int32_default dim) 
{
    solver_int32_default i;
    for( i = 0; i < dim; i++ ) 
    {
        *dest++ = (dynamics_solver_forcespro_drl_float) (*src++) ;
    }
}

void copyMValueToC_dynamics_solver_forcespro_drl_float(double * src, dynamics_solver_forcespro_drl_float * dest)
{
	*dest = (dynamics_solver_forcespro_drl_float) *src;
}

extern solver_int32_default (dynamics_solver_forcespro_drl_float *x, dynamics_solver_forcespro_drl_float *y, dynamics_solver_forcespro_drl_float *l, dynamics_solver_forcespro_drl_float *p, dynamics_solver_forcespro_drl_float *f, dynamics_solver_forcespro_drl_float *nabla_f, dynamics_solver_forcespro_drl_float *c, dynamics_solver_forcespro_drl_float *nabla_c, dynamics_solver_forcespro_drl_float *h, dynamics_solver_forcespro_drl_float *nabla_h, dynamics_solver_forcespro_drl_float *hess, solver_int32_default stage, solver_int32_default iteration, solver_int32_default threadID);
dynamics_solver_forcespro_drl_extfunc pt2function_dynamics_solver_forcespro_drl = &;


/* Some memory for MEX function */
static dynamics_solver_forcespro_drl_params params;
static dynamics_solver_forcespro_drl_output output;
static dynamics_solver_forcespro_drl_info info;
static dynamics_solver_forcespro_drl_mem * mem;
...
void mexFunction( solver_int32_default nlhs, mxArray *plhs[], solver_int32_default nrhs, const mxArray *prhs[] )  
{
    ...
    if (nrhs != 1)
	{
		mexErrMsgTxt("This function requires exactly 1 input: PARAMS struct.\nType 'help dynamics_solver_forcespro_drl_mex' for details.");
	}    
	if (nlhs > 3) 
	{
    ...
    if (mem == NULL)
    {
        mem = dynamics_solver_forcespro_drl_internal_mem(0);
    }
    ...
    if( mxGetM(par) != 805 || mxGetN(par) != 1 ) 
	{
    mexErrMsgTxt("PARAMS.all_parameters must be of size [805 x 1]");
    }
#endif	 
	if ( (mxGetN(par) != 0) && (mxGetM(par) != 0) )
	{
		copyMArrayToC_double(mxGetPr(par), params.all_parameters,805);

	}


	#if SET_PRINTLEVEL_dynamics_solver_forcespro_drl > 0
	...
	exitflag = dynamics_solver_forcespro_drl_solve(&params, &output, &info, mem, fp, pt2function_dynamics_solver_forcespro_drl);
	...
	*mxGetPr(plhs[1]) = (double)exitflag;
	...
	plhs[2] = mxCreateStructMatrix(1, 1, 20, infofields);
		...
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.res_eq)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "res_eq", outvar);


		/* scalar: inf-norm of inequality constraint residuals */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.res_ineq)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "res_ineq", outvar);


		/* scalar: norm of stationarity condition */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.rsnorm)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "rsnorm", outvar);


		/* scalar: max of all complementarity violations */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.rcompnorm)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "rcompnorm", outvar);


		/* scalar: primal objective */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.pobj)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "pobj", outvar);


		/* scalar: dual objective */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.dobj)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "dobj", outvar);


		/* scalar: duality gap := pobj - dobj */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.dgap)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "dgap", outvar);


		/* scalar: relative duality gap := |dgap / pobj | */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.rdgap)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "rdgap", outvar);


		/* scalar: duality measure */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.mu)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "mu", outvar);


		/* scalar: duality measure (after affine step) */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.mu_aff)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "mu_aff", outvar);


		/* scalar: centering parameter */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.sigma)), mxGetPr(outvar), 1);
		...
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.step_aff)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "step_aff", outvar);


		/* scalar: step size (combined direction) */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.step_cc)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "step_cc", outvar);


		/* scalar: total solve time */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.solvetime)), mxGetPr(outvar), 1);
		mxSetField(plhs[2], 0, "solvetime", outvar);


		/* scalar: time spent in function evaluations */
		outvar = mxCreateDoubleMatrix(1, 1, mxREAL);
		copyCArrayToM_dynamics_solver_forcespro_drl_float((&(info.fevalstime)), mxGetPr(outvar), 1);
