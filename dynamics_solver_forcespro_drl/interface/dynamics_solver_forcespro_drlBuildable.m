...
% 
classdef dynamics_solver_forcespro_drlBuildable < coder.ExternalDependency
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, cfg)
            ...
            if(~strcmp(foldername, 'interface') || ~strcmp(solvername, 'dynamics_solver_forcespro_drl'))
                solverpath = fullfile(buildablepath, 'dynamics_solver_forcespro_drl');
            end
            solverInfo = struct();
            solverInfo.solvername = 'dynamics_solver_forcespro_drl';
        ...
        function [output,exitflag,info] = forcesCallWithParams(params)
            [output,exitflag,info] = dynamics_solver_forcespro_drlBuildable.forcesCall(params.x0, params.xinit, params.all_parameters);
        end

        function [output, exitflag, info] = forcesCall(x0, xinit, all_parameters)
            solvername = 'dynamics_solver_forcespro_drl';
            ...
            [output_c, exitflag_c, info_c] = dynamics_solver_forcespro_drlBuildable.forcesInitOutputsC(); %#ok<ASGLU>
            ...
            [output, exitflag, info] = dynamics_solver_forcespro_drlBuildable.forcesInitOutputsMatlab();
