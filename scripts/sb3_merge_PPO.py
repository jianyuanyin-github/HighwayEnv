...
if __name__ == "__main__":
    ...
    def signal_handler(signum, frame):
        ...
        if current_model is not None:
            ...
            emergency_save_path = f"merge_ppo/model_emergency_{timestamp}"
    ...
    env = make_vec_env("merge-v0", n_envs=n_envs, vec_env_cls=SubprocVecEnv)
    ...
    if TRAIN:
        ...
        checkpoint_callback = CheckpointCallback(
            save_freq=10000,
            save_path="./merge_ppo/checkpoints/",
        ...
        timestamp = int(time.time())
        model_name = f"merge_ppo/model_{timestamp}"
    ...
    if MODEL_TO_LOAD == "latest":
        ...
        model_files = glob.glob("merge_ppo/model_*.zip")
        if model_files:
            ...
            model_path = "merge_ppo/model.zip"
        ...
        if not MODEL_TO_LOAD.endswith(".zip"):
            model_path = f"merge_ppo/{MODEL_TO_LOAD}.zip"
        else:
            model_path = f"merge_ppo/{MODEL_TO_LOAD}"
    ...
    try:
        ...
        all_models = glob.glob("merge_ppo/model*.zip")
        for m in all_models:
            print(f"  {m}")
        exit(1)
