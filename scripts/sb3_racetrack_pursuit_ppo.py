...
if __name__ == "__main__":
    ...
    def signal_handler(signum, frame):
        ...
        if current_model is not None:
            ...
            emergency_save_path = f"racetrack_pursuit_ppo/model_emergency_{timestamp}"
    ...
    if CONTINUE_TRAINING:
        try:
            model = PPO.load(
                "racetrack_pursuit_ppo/model",
                env=env,
                tensorboard_log="racetrack_pursuit_ppo/",
            )
    ...
    if TRAIN:
        ...
        # Save checkpoint every 80,000 steps (5 actions: SLOWER/IDLE/FASTER/LANE_LEFT/LANE_RIGHT)
        checkpoint_callback = CheckpointCallback(
            save_freq=10000,
            save_path="./racetrack_pursuit_ppo/checkpoints/",
        ...
        timestamp = int(time.time())
        model_name = f"racetrack_pursuit_ppo/model_{timestamp}"
    ...
    if MODEL_TO_LOAD == "latest":
        ...
        model_files = glob.glob("racetrack_pursuit_ppo/model_*.zip")
        if model_files:
            ...
            model_path = "racetrack_pursuit_ppo/model.zip"
        ...
        if not MODEL_TO_LOAD.endswith(".zip"):
            model_path = f"racetrack_pursuit_ppo/{MODEL_TO_LOAD}.zip"
        else:
            model_path = f"racetrack_pursuit_ppo/{MODEL_TO_LOAD}"
    ...
    try:
        ...
        all_models = glob.glob("racetrack_pursuit_ppo/model*.zip")
        for m in all_models:
            print(f"  {m}")
        exit(1)

    env = gym.make("racetrack-single-pursuit-v0", render_mode="rgb_array")
    env = RecordVideo(
        env, video_folder="racetrack_pursuit_ppo/videos", episode_trigger=lambda e: True
    )
