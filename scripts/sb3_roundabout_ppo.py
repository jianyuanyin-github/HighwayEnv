...
MODEL_TO_LOAD = "model_emergency_1756903574"  # "latest": load newest model, "model": load model.zip, or specific filename like "model_1234567890"
...
if __name__ == "__main__":
    ...
    def signal_handler(signum, frame):
        ...
        if current_model is not None:
            ...
            emergency_save_path = f"roundabout_ppo/model_emergency_{timestamp}"
    ...
    env = make_vec_env("roundabout-v0", n_envs=n_envs, vec_env_cls=SubprocVecEnv)

    # Decide whether to continue training based on configuration
    if CONTINUE_TRAINING:
        try:
            model = PPO.load(
                "roundabout_ppo/model",
                env=env,
    ...
    if TRAIN:
        ...
        checkpoint_callback = CheckpointCallback(
            save_freq=10000,
            save_path="./roundabout_ppo/checkpoints/",
        ...
        timestamp = int(time.time())
        model_name = f"roundabout_ppo/model_{timestamp}"
    ...
    if MODEL_TO_LOAD == "latest":
        ...
        model_files = glob.glob("roundabout_ppo/model_*.zip")
        if model_files:
            ...
            model_path = "roundabout_ppo/model.zip"
        ...
        if not MODEL_TO_LOAD.endswith(".zip"):
            model_path = f"roundabout_ppo/{MODEL_TO_LOAD}.zip"
        else:
            model_path = f"roundabout_ppo/{MODEL_TO_LOAD}"
    ...
    try:
        ...
        all_models = glob.glob("roundabout_ppo/model*.zip")
        for m in all_models:
            print(f"  {m}")
        exit(1)
    ...
    env = RecordVideo(
        env, video_folder="roundabout_ppo/videos", episode_trigger=lambda e: True
    )
