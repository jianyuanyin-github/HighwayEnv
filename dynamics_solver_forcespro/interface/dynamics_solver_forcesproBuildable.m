...
% 
classdef dynamics_solver_forcesproBuildable < coder.ExternalDependency
    ...
    methods (Static)
        ...
        function updateBuildInfo(buildInfo, cfg)
            ...
            if(~strcmp(foldername, 'interface') || ~strcmp(solvername, 'dynamics_solver_forcespro'))
                solverpath = fullfile(buildablepath, 'dynamics_solver_forcespro');
            end
            solverInfo = struct();
            solverInfo.solvername = 'dynamics_solver_forcespro';
        ...
        function [output,exitflag,info] = forcesCallWithParams(params)
            [output,exitflag,info] = dynamics_solver_forcesproBuildable.forcesCall(params.x0, params.xinit, params.all_parameters);
        end

        function [output, exitflag, info] = forcesCall(x0, xinit, all_parameters)
            solvername = 'dynamics_solver_forcespro';
            ...
            [output_c, exitflag_c, info_c] = dynamics_solver_forcesproBuildable.forcesInitOutputsC(); %#ok<ASGLU>
            ...
            [output, exitflag, info] = dynamics_solver_forcesproBuildable.forcesInitOutputsMatlab();
