...
% 
% dynamics_solver_forcespro_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME)
%
% This function generates a Simulink Model with Fixed-step solver type named MODELNAME to add the dynamics_solver_forcespro FORCESPRO solver as 
% a MATLAB Function Simulink Block named BLOCKNAME so that it can be used for Simulink simulation and 
% Simulink Code Generation.
%
% dynamics_solver_forcespro_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK)
% can be used to generate a MATLAB Function Simulink Block with compact inputs and outputs.
%
% dynamics_solver_forcespro_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL)
% can be used if the selected Simulink Model already exists in order to add the Simulink Block to it.
%
% dynamics_solver_forcespro_CREATECODERMATLABFUNCTION(MODELNAME, BLOCKNAME, USECOMPACTBLOCK, USEEXISTINGMODEL, OPENSIMULINKMODEL)
% can be used to select whether to open the simulink model.
function dynamics_solver_forcespro_createCoderMatlabFunction(modelname, blockname, useCompactBlock, useExistingModel, openSimulinkModel)

    if nargin < 1 || isempty(modelname)
        modelname = 'dynamics_solver_forcespro_model';
    end
    if ~isvarname(modelname)
        error('Modelname must be a valid variable name');
    end
    if nargin < 2 || isempty(blockname)
        blockname = 'dynamics_solver_forcespro_block';
    end
    ...
    function_name = 'dynamics_solver_forcespro_coderFunction.m';
    position = [170, 99, 650, 775];
    parameter_sizes = struct('x0', '[420,1]',...
                             'xinit', '[9,1]',...
                             'all_parameters', '[595,1]');
    if useCompactBlock
        function_name = 'dynamics_solver_forcespro_coderFunctioncompact.m';
